...
- **The ring belongs to the field, not the bare input.** A borderless input inside a bordered row gets a floating rectangle that fights the row's own underline or border. Put the indicator on `.mc4wp-form-fields:focus-within` and reset the input's own outline with `& input:not([type=submit]):focus{outline:none}` — note the `:not([type=submit])`, or that reset silently kills the submit's ring too (equal specificity, later rule wins).
- **Form controls do not inherit `color`.** A submit styled with `color:transparent` to hide its label makes `outline:2px solid currentColor` an *invisible* ring, and inheriting nothing makes it the UA's black. Hide the label with `font-size:0;line-height:0` and set `color:inherit`, so `currentColor` is the surface's real text colour and the ring adapts to every style variation.
- **`:focus-visible` is not allowed in theme.json** (`VALID_ELEMENT_PSEUDO_SELECTORS` permits only `:link`, `:any-link`, `:visited`, `:hover`, `:focus`, `:active`), so `elements.button:focus` also fires on mouse clicks and leaves a stuck ring. Top-level `styles.css` closes the gap with one rule — `:where(…):focus:not(:focus-visible){outline:none}` — whose `(0,2,0)` specificity beats the `:root :where(…)` `(0,1,0)` that theme.json emits. For the same reason `elements.button:focus` must **not** change colours: a click would leave the button stuck in that state.

Button rings use `outline-offset: -4px` (inset) on purpose: an *outer* `currentColor` ring is white on a filled button and disappears against a white page. Drawn inside, it always sits over the button's own fill.
