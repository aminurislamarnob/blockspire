...
> **theme.json cannot outrank WooCommerce's own CSS.** Everything under `styles.blocks.<block>.css` is emitted wrapped in `:root :where(…)`, which is specificity **(0,1,0)** no matter how many classes the inner selector names — `:where()` zeroes them. WooCommerce's compatibility CSS routinely sits at (0,2,0) or (0,2,1) (`.woocommerce span.onsale`, `.woocommerce-cart .wp-block-post-title{max-width:1000px}`), so it wins every time. Rules that must beat the plugin belong in top-level `styles.css`, which is emitted **verbatim** — the same escape hatch the focus states use. Keep them scoped to a WooCommerce body or block class so they stay inert without the plugin. Rules that only have to beat *core* defaults (the sale-badge pill on `woocommerce/product-sale-badge`, the 8px image radius, the struck-through price colour) are fine in `styles.blocks` and are then skipped entirely when WooCommerce is inactive, because unregistered blocks get no selector.
